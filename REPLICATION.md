...
This document explains how the project replicates the `https://connectionssolver.com` homepage and validates visual parity. Follow these steps whenever you need to resync the clone or rerun the automated checks.
...
2. Extract the `<head>` and `<body>` fragments into the Next.js data directory:
   ...
   ```
3. Regenerate the static snapshot used for baseline comparisons:
   ```bash
